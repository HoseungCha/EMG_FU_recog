function output = possible_fe_selector(score_matrix,output_matrix,name_gesture,n_part)
...
gesture_selected = ...
    name_gesture{part_selected}{output_matrix(part_selected,class_selector(part_selected))};

% gesture_group_to_be_checked...
%     = name_gesture{part_to_be_checked,class_selector(part_to_be_checked)};
gesture_to_be_checked = ...
    name_gesture{part_to_be_checked}{...
    output_matrix(...
    part_to_be_checked,class_selector(part_to_be_checked))};

gesture_possible_other_part= name_gesture{part_to_be_checked}( ...
     ~cellfun(@isempty,strfind(name_gesture{part_selected},gesture_selected)));
