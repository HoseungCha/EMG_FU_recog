...
name_trg = {"angry(eye brow wrinkled, lip tightening)",...
    ...
    "찡그림(nose wrinkled)",5;"두려움(lip strecher)",...
        ...
        for i_trl_test = find(idx_trl~=i_trl)
            ...
            for i_fe = 1: n_FE
                ...
                for i_cf = 1 : n_cf
                    ...
                    tmp_RMS = tmp_feat(1:n_seg2use,[1,8]);

                    % EMG onset detection
                    output_EMGonset = zeros(n_seg2use,2);
                    id_emg_onset_chan = NaN(1,2);
                    for i_ch = 1 : 2
                        output_EMGonset(:,i_ch) = ...
                            predict(model_tree_emg_onset,tmp_RMS(:,i_ch));
                        [~,tmp_idx] = max(countmember(...
...
% y = get_output_within_possible_outcomes(possible_eyebrows,output_test)
% counts outputs within possible outcomes
% outcome_counts = ...
%     countmember(possible_eyebrows,output_test{1});
    % randomlly pick one as eye-brow expression
%     ouput_mv(1) = possible_outcomes...
%         (idx_max(randi(length(idx_max))));
% else
%     [~,tmp_idx] = max(outcome_counts);
%     ouput_mv(1) = possible_outcomes(tmp_idx);
